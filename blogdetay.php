...
<html lang="tr">
   <head>
      ...
      <meta http-equiv="X-UA-Compatible" content="IE=edge">
	        ...
	        <!-- Title -->
      <title><?=$sbilgi['baslik']?></title>
      <meta name="description" content="<?=$sbilgi['ozet']?>">
...
<img src="<?=$row['adres']?>" style="margin:10px;">
</div>
<?php } ?>
</div>
<?php } ?>
			   ...
			   <div class="col-md-4">
					<div class="single-candidate-bottom-left">
                      <div class="single-candidate-widget" style="background:#fff;padding:20px;">
                        <h3>Diğer Yazılar</h3>
<?php
$query = $db->prepare("SELECT * FROM sayfalar where etiket like '%blog%' and link<>'".$sbilgi['link']."' order by id desc");
$query->execute();
$verisay = $query->rowCount();
if($verisay){
while($row=$query->fetch(PDO::FETCH_ASSOC)) {
?>
<p><a href="/blog/<?=$row['link']?>"><i class="fa fa-arrow-right"></i> <?=$row['baslik']?></a></p>
